...
TODO: mention savaging and versioning.
...
- close :: Closing the file during disconnection actually has the same semantics as when connected: changes are persisted locally on disk, but no guarantee that the owner accepts the change.
