...
\date{\today}
\title{File Sharing with Minimum Responsibility}
\hypersetup{
 pdfauthor={Yuan Fu},
 pdftitle={File Sharing with Minimum Responsibility},
 pdfkeywords={},
 pdfsubject={},
...
\section{Abstract}
\label{sec:orgb7e91e2}

\section{Introduction}
\label{sec:org06d97f1}
...
\label{sec:org276fabf}
\subsection{Modularization}
\label{sec:orgd557437}
...
\label{sec:org42a311e}

\begin{description}
\item[{attr(file)}] Return the attribute of \(\mathup{file}\).
\item[{read(file, offset, size)}] Read \(file\).
\item[{write(file, offset, data)}] Write \(file\).
\item[{create(parent, name, type)}] Create a file with \(name\) under \(parent\), \(type\) can be file or directory.
\item[{open(file)}] Open \(file\).
\item[{close(file)}] Close \(file\).
\item[{delete(file)}] Delete file.
\item[{readdir(dir)}] List entries in \(dir\).
\end{description}

\subsection{RPC interface}
\label{sec:org2a2be69}

Our rpc interface contains all the function in the previous listing, plus:

\begin{description}
\item[{savage(vault, file)}] Savage \(file\) in \(vault\). The receiver of this request doesn’t have to be the owner of \(vault\). This request returns both file content and file version.
\item[{submit(file, version)}] Submits \(file\) at \(version\) to the owner.
\end{description}

\section{Evaluation}
\label{sec:org661163f}

TBC

\section{Future work}
\label{sec:org35d2a10}
